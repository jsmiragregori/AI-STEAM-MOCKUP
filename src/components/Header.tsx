import { Globe, User, BookOpen, MessageSquare } from 'lucide-react';
import { Tab } from '../App';
    ...
    <div className="fixed top-0 left-0 right-0 z-50">
      ...
      <header className="bg-white border-b border-eu-border h-16 flex items-center justify-between px-6">
        ...
        >
          ...
          <span className="font-bold text-xl text-eu-blue">AI-STEAM Network</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden lg:flex items-center gap-2 border-l border-eu-border pl-4 ml-4">
            <button className="bg-gray-200 border-none px-2.5 py-1 rounded text-[11px] font-bold text-gray-700 cursor-pointer hover:bg-gray-300">
              Aules
            </button>
            <button className="bg-gray-200 border-none px-2.5 py-1 rounded text-[11px] font-bold text-gray-700 cursor-pointer hover:bg-gray-300">
              ConsensUE
      ...
      {/* Nav Menu */}
      <nav className="bg-eu-blue h-12 flex px-6 gap-6">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveTab(item.id)}
            className={`text-sm font-medium flex items-center cursor-pointer border-b-[3px] transition-all duration-200 ${
              activeTab === item.id
